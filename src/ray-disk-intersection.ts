...
export function intersectDisk(disk:Disk, ray:{ origin:number[]|vec3, dir:number[]|vec3 }) : number|null {
    const t = intersectPlane({n: disk.n, p: disk.p}, ray);
    if (t) {
        const p = vec3.scaleAndAdd(vec3.create(), ray.origin as ReadonlyVec3, ray.dir as ReadonlyVec3, t);
